## Just some cute cartoon cat sways its paws


### Knows rules
 - bilibili_cat (BiliBili) (哔哩哔哩)
 - doro         (Doro)
 - beyond-Journey_s-End (葬送のフリーレン) (葬送的芙莉莲)
 - kasane_teto (かさねてと) (重音teto)
 - maodie (A very bad cat) (一只会哈气的猫)
 - miku_cat (はつね みく) (初音未来)
 - murder_drones (Murder Drones) (无机杀手)
 - palworld (Palworld) (幻兽帕鲁)
 - plants_vs_zombies (PVZ) (植物大战僵尸)
 - undertale (undertale) (传说之下)
 - windows_cat (Microsoft Windows)
 - youshouyan (有兽焉)
 - kipfel (キプフェル)
 - blue_archive (ブルーアーカイブ) (Blue Archive) (蔚蓝档案)
 - unknown (I have an impression, but I forget what it's called)
